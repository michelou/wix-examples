...
set __WIX_GUIDS=PRODUCT_CODE UPGRADE_CODE MAIN_EXECUTABLE PROGRAM_MENU_DIR

set __APP_GUIDS=VERSION COMMON_BAT COMMON_SH REPL_BAT APPLICATION_SHORTCUT APPLICATION_ENV
set __APP_GUIDS=%__APP_GUIDS% SCALA_BAT SCALA_SH SCALAC_BAT SCALAC_SH SCALADOC_BAT SCALADOC_SH
set __APP_GUIDS=%__APP_GUIDS% ANTLR_JAR ANTLR_RUNTIME_JAR AUTOLINK_JAR COMPILER_INTERFACE_JAR
set __APP_GUIDS=%__APP_GUIDS% DIST_3_JAR FLEXMARK_JAR FLEXMARK_EXT_ANCHORLINK_JAR FLEXMARK_EXT_AUTOLINK_JAR
set __APP_GUIDS=%__APP_GUIDS% FLEXMARK_EXT_EMOJI_JAR FLEXMAKR_EXT_GFM_STRIKETHROUGH_JAR FLEXMARK_EXT_GFM_TABLES_JAR
set __APP_GUIDS=%__APP_GUIDS% FLEXMARK_EXT_GFM_TASKLIST_JAR FLEXMARK_EXT_INS_JAR FLEXMARK_EXT_SUPERSCRIPT_JAR
set __APP_GUIDS=%__APP_GUIDS% FLEXMARK_EXT_TABLES_JAR FLEXMARK_EXT_WIKILINK_JAR FLEXMARK_EXT_YAML_FRONT_MATTER_JAR
set __APP_GUIDS=%__APP_GUIDS% FLEXMARK_FORMATTER_JAR FLEXMARK_HTML_PARSER_JAR FLEXMAKR_JIRA_CONVERTER_JAR FLEXMARK_UTIL_JAR
set __APP_GUIDS=%__APP_GUIDS% JACKSON_ANNOTATIONS_JAR JACKSON_CORE_JAR JACKSON_DATABIND_JAR JACKSON_DATAFORMAT_YAML_JAR
set __APP_GUIDS=%__APP_GUIDS% JLINE_READER_JAR JLINE_TERMINAL_JAR JLINE_TERMINAL_JNA_JAR JNA_JAR
set __APP_GUIDS=%__APP_GUIDS% JSOUP_JAR LIQP_JAR PROTOBUF_JAVA_JAR SCALA_ASM_JAR SCALA_LIBRARY_JAR
set __APP_GUIDS=%__APP_GUIDS% SCALA3_COMPILER_JAR SCALA3_INTERFACES_JAR SCALA3_LIBRARY_JAR
set __APP_GUIDS=%__APP_GUIDS% SCALA3_STAGING_3_JAR SCALA3_TASTY_INSPECTOR_JAR
set __APP_GUIDS=%__APP_GUIDS% SCALADOC_3_JAR SNAKEYAML_JAR ST4_JAR TASTY_CORE_JAR UTIL_INTERFACE_JAR
...
for %%i in (%__WIX_GUIDS% %__APP_GUIDS%) do (
...
if not exist "%_SOURCE_DIR%\localizations\*.wxl" (
    echo %_ERROR_LABEL% No localization file found in directory "!_SOURCE_DIR=%_ROOT_DIR%=!localizations\" 1>&2
    set _EXITCODE=1
    goto :eof
)
for /f "delims=" %%f in ('dir /b /s "%_SOURCE_DIR%\localizations\*.wxl"') do (
    set "__CULTURES=%%~nf"
