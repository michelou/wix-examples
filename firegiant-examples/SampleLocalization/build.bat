...
:args
set _CLEAN=0
set _CULTURES=%_CULTURES_DEFAULT%
set _HELP=0
set _INSTALL=0
set _LINK=0
set _REMOVE=0
...
if "%__ARG:~0,1%"=="-" (
    ...
    ) else if "%__ARG%"=="-loc:de-de" ( set _CULTURES=de-de
    ) else if "%__ARG%"=="-loc:en-us" ( set _CULTURES=en-us
    ) else if "%__ARG%"=="-loc:fr-fr" ( set _CULTURES=fr-fr
...
if not exist "%_SOURCE_DIR%\localization\%_CULTURES%.wxl" (
    echo %_ERROR_LABEL% Localization file not found ^(%_CULTURES%^) 1>&2
    set _EXITCODE=1
    goto :eof
)
set "_LOCALIZATION_FILE=%_SOURCE_DIR%\localization\%_CULTURES%.wxl"
set "_MSI_FILE=!_MSI_FILE:.msi=_%_CULTURES%.msi!"

if %_DEBUG%==1 (
    echo %_DEBUG_LABEL% Options    : _CULTURES=%_CULTURES% _TIMER=%_TIMER% _VERBOSE=%_VERBOSE% 1>&2
...
echo %__OPT_VERBOSE% %__OPT_EXTENSIONS% %__OPT_PROPERTIES% -nologo -out "%_TARGET_DIR:\=\\%\\"> "%__OPTS_FILE%"
...
if defined _CULTURES (
    set __OPT_LOC=-cultures:%_CULTURES% -loc "%_LOCALIZATION_FILE%"
    set "__OPTS_FILE=%_TARGET_DIR%\light_opts_%_CULTURES%.txt"
) else (
    set __OPT_LOC=
...
echo %__OPT_VERBOSE% %__OPT_EXTENSIONS% %__OPT_PROPERTIES% -nologo %__OPT_LOC:\=\\% -out "%_MSI_FILE:\=\\%" %__LIGHT_BINDINGS%> "%__OPTS_FILE%"
